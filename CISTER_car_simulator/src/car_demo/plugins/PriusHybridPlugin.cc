...
/*
 * Copyright 2018 Pedro Santos (pjsol@isep.ipp.pt)
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
...
//#define CARINFO_PERIOD 0.05 //0.001
//#define CAM_PERIOD 0.1

#define CARINFO_PERIOD 0.01 //0.001
#define CAM_PERIOD 0.03
...
namespace gazebo
{
  class PriusHybridPluginPrivate
  {
    ...
    /// \brief X_enviroment Network Pub
    public: ros::Publisher XNetwork_Pub;

    /// \brief X_enviroment Network Sub
    public: ros::Subscriber XNetwork_Sub;

    /// \brief RX Network Pub
    public: ros::Publisher RXNetwork_Pub;

    /// \brief TX Network Pub
    public: ros::Subscriber TXNetwork_Sub;

    /// \brief carINFO Pub
    public: ros::Publisher carINFO_Pub;

    public: ros::Publisher omnetCAM_Pub;

    /// \brief Last sim time when a CAM Message was sent
    public: common::Time lastCAM_MsgSentTime;

    /// \brief Last sim time when a carINFO Message was sent
    public: common::Time last_carINFO_MsgSentTime;
...
/////////////////ROS COMM between cars////////////////////////////
void PriusHybridPlugin::OnNetworkRX(const ros_its_msgs::CAM_simplified::ConstPtr& msg)
{ 
...
////////////////////////////////////////////////////////////////
void PriusHybridPlugin::OnNetworkTX(const ros_its_msgs::CAM_simplified::ConstPtr& msg)
{
  this->dataPtr->XNetwork_Pub.publish(msg);
  return;
}
...
void PriusHybridPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
  ...
  if (_sdf->HasElement("robotNamespace"))
    this->dataPtr->robot_namespace_ = _sdf->GetElement("robotNamespace")->Get<std::string>();
  //*****
  ...
  //Network 
  this->dataPtr->XNetwork_Sub   = nh2.subscribe("XNetwork", 10, &PriusHybridPlugin::OnNetworkRX, this);
  this->dataPtr->XNetwork_Pub   = nh2.advertise<ros_its_msgs::CAM_simplified>("XNetwork", 1000);
  this->dataPtr->RXNetwork_Pub  = nh.advertise<ros_its_msgs::CAM_simplified>("RXNetwork", 1000);
  this->dataPtr->carINFO_Pub    = nh.advertise<ros_its_msgs::CAM_simplified>("carINFO", 1000);
  this->dataPtr->TXNetwork_Sub  = nh.subscribe("TXNetwork", 10, &PriusHybridPlugin::OnNetworkTX, this);
  this->dataPtr->omnetCAM_Pub    = nh.advertise<ros_its_msgs::OMNET_CAM>("omnetCAM", 1000);
  //////////
  md5("grape");

  this->dataPtr->node.Subscribe(this->dataPtr->robot_namespace_ + "prius/reset", &PriusHybridPlugin::OnReset, this);
  this->dataPtr->node.Subscribe(this->dataPtr->robot_namespace_ + "prius/stop", &PriusHybridPlugin::OnStop, this);

  this->dataPtr->node.Subscribe(this->dataPtr->robot_namespace_ + "cmd_vel", &PriusHybridPlugin::OnCmdVel, this);
  this->dataPtr->node.Subscribe(this->dataPtr->robot_namespace_ + "cmd_gear", &PriusHybridPlugin::OnCmdGear, this);
  this->dataPtr->node.Subscribe(this->dataPtr->robot_namespace_ + "cmd_mode", &PriusHybridPlugin::OnCmdMode, this);

  
  this->dataPtr->posePub = this->dataPtr->node.Advertise<ignition::msgs::Pose>(this->dataPtr->robot_namespace_ +"prius/pose");
  this->dataPtr->consolePub = this->dataPtr->node.Advertise<ignition::msgs::Double_V>(this->dataPtr->robot_namespace_ + "prius/console");
  ...
  if(this->dataPtr->robot_namespace_ == "/car1/"){ //only control front car TODO: variable leader!!
    this->dataPtr->keyboardSub = this->dataPtr->gznode->Subscribe("~/keyboard/keypress",
...
void PriusHybridPlugin::Reset()
{
  ...
  this->dataPtr->lastCAM_MsgSentTime = 0;
  this->dataPtr->last_carINFO_MsgSentTime = 0;
}
...
void PriusHybridPlugin::Update()
{
  ...
  if (curTime - this->dataPtr->lastCAM_MsgSentTime > CAM_PERIOD)
  {
    ...
    //Send CAM message
    ros_its_msgs::CAM_simplified CAM_msg;
    ...
    CAM_msg.speed_speedValue = this->dataPtr->chassisLinearVelocity.Length(); //km/h
    CAM_msg.driveDirection = this->dataPtr->directionState; //forward(0), backward(1), unavailable(2)
    CAM_msg.steeringWheelAngle_steeringWheelAngleValue = (this->dataPtr->flSteeringAngle + this->dataPtr->frSteeringAngle) / 2;
    CAM_msg.gasPedalPercent_Value = this->dataPtr->gasPedalPercent;
    CAM_msg.brakePedalPercent_Value = this->dataPtr->brakePedalPercent;

    this->dataPtr->XNetwork_Pub.publish(CAM_msg);
  }
  if (curTime - this->dataPtr->last_carINFO_MsgSentTime > CARINFO_PERIOD)
  {
    this->dataPtr->last_carINFO_MsgSentTime = curTime;
    //Send CAM message
    ros_its_msgs::CAM_simplified carINFO_msg;
    ros_its_msgs::OMNET_CAM CAM_msg;
    ignition::math::Pose3d pose = dPtr->chassisLink->WorldPose();

    carINFO_msg.car_name = this->dataPtr->robot_namespace_;
    carINFO_msg.latitude = pose.Pos().X();
    carINFO_msg.longitude = pose.Pos().Y();
    carINFO_msg.altitude_altitudeValue = pose.Pos().Z();
    carINFO_msg.heading_headingValue = pose.Rot().Yaw();
    carINFO_msg.speed_speedValue = this->dataPtr->chassisLinearVelocity.Length(); //km/h
    carINFO_msg.driveDirection = this->dataPtr->directionState; //forward(0), backward(1), unavailable(2)
    carINFO_msg.steeringWheelAngle_steeringWheelAngleValue = (this->dataPtr->flSteeringAngle + this->dataPtr->frSteeringAngle) / 2;
    carINFO_msg.gasPedalPercent_Value = this->dataPtr->gasPedalPercent;
    carINFO_msg.brakePedalPercent_Value = this->dataPtr->brakePedalPercent;

    this->dataPtr->carINFO_Pub.publish(carINFO_msg);
    
    CAM_msg.car_name = this->dataPtr->robot_namespace_;
    CAM_msg.latitude = pose.Pos().X();
    CAM_msg.longitude = pose.Pos().Y();
    CAM_msg.altitude = pose.Pos().Z();
    CAM_msg.heading = pose.Rot().Yaw();
    CAM_msg.speed = this->dataPtr->chassisLinearVelocity.Length();
	if (this->dataPtr->gasPedalPercent > 0){
		CAM_msg.acceleration = this->dataPtr->gasPedalPercent;
	} else{
		CAM_msg.acceleration = -(this->dataPtr->brakePedalPercent);
	}
    
    this->dataPtr->omnetCAM_Pub.publish(CAM_msg);
