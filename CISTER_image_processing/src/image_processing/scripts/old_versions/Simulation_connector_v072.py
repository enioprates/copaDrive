...
def trata_sonar():
	...
	if (range_RM < MAX_RANGE or range_LM < MAX_RANGE or range_LF < MAX_RANGE or range_RF < MAX_RANGE):
		if (range_LF < MAX_RANGE and range_RF < MAX_RANGE):
			obstacle = FRENTE
		elif range_LF < MAX_RANGE:
			obstacle = ESQUERDA
		elif  range_RF < MAX_RANGE:
			obstacle = DIREITA
		else:
			obstacle = INDEFINIDO
...
def listener():
	...
	rospy.Subscriber('/car1/front_sonar_left_far_range',Sonar,sonar_LF)#subscreve ao topicoi e chama a funcao sonar_LF
	rospy.Subscriber('/car1/front_sonar_left_middle_range',Sonar,sonar_LM)#subscreve ao topicoi e chama a funcao sonar_LF
	rospy.Subscriber('/car1/front_sonar_right_middle_range',Sonar,sonar_RM)#subscreve ao topicoi e chama a funcao sonar_LF
	rospy.Subscriber('/car1/front_sonar_right_far_range',Sonar,sonar_RF)#subscreve ao topicoi e chama a funcao sonar_LF
