...
kp_theta = 2.0      #9.0    #6.0
ki_theta = 0.005     #0.1    #0.05
kd_theta = 0.5      #3.5    #3.0
...
def compare_lost_position():
    ...
    #Find SV direction
    if (abs(delta_longitude_SV) <= 0.1 and delta_latitude_SV < 0.0):
        direction = WEST
        if debug:
            print "DIR: WEST - ", direction 
    elif (delta_longitude_SV > 0.0 and delta_latitude_SV < 0.0):
        direction = NW
        if debug:
            print "DIR: - NW", direction
    elif (delta_longitude_SV > 0.0 and abs(delta_latitude_SV) <= 0.1):
        direction = NORTH
        if debug:
    ...
    elif (abs(delta_longitude_SV) <= 0.1 and delta_latitude_SV > 0.0):
        direction = EST
        if debug:
