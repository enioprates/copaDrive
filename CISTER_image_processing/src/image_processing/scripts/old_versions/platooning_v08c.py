...
def compare_lost_position():
    ...
    for i in range(TV_position_vector.shape[0]):
        ...
        if (abs(delta_longitude_SV) <= 0.1 and delta_latitude_SV < 0.0 and TV_position_vector[i,0] < latitude_SV):                                             #1
            dist = ((TV_position_vector[i,1] - longitude_SV)**2 + (TV_position_vector[i,0] - latitude_SV)**2)**0.5
            if debug:
                print "1"
        elif (delta_longitude_SV > 0.0 and delta_latitude_SV < 0.0 and TV_position_vector[i,0] < latitude_SV and TV_position_vector[i,1] > longitude_SV):   #2
            dist = ((TV_position_vector[i,1] - longitude_SV)**2 + (TV_position_vector[i,0] - latitude_SV)**2)**0.5
            if debug:
                print "2"
        elif (delta_longitude_SV > 0.0 and abs(delta_latitude_SV) <= 0.1 and TV_position_vector[i,1] > longitude_SV):                                           #3
            dist = ((TV_position_vector[i,1] - longitude_SV)**2 + (TV_position_vector[i,0] - latitude_SV)**2)**0.5
            if debug:
                print "3"
        elif (delta_longitude_SV > 0.0 and delta_latitude_SV > 0.0 and TV_position_vector[i,0] > latitude_SV and TV_position_vector[i,1] > longitude_SV):   #4
            dist = ((TV_position_vector[i,1] - longitude_SV)**2 + (TV_position_vector[i,0] - latitude_SV)**2)**0.5
            if debug:
                print "4"
        elif (abs(delta_longitude_SV) <= 0.1 and delta_latitude_SV > 0.0 and TV_position_vector[i,0] > latitude_SV):                                            #5
            dist = ((TV_position_vector[i,1] - longitude_SV)**2 + (TV_position_vector[i,0] - latitude_SV)**2)**0.5
            if debug:
                print "5"
        elif (delta_longitude_SV < 0.0 and delta_latitude_SV > 0.0 and TV_position_vector[i,0] > latitude_SV and TV_position_vector[i,1] < longitude_SV):   #6
            dist = ((TV_position_vector[i,1] - longitude_SV)**2 + (TV_position_vector[i,0] - latitude_SV)**2)**0.5
            if debug:
                print "6"
        elif (delta_longitude_SV < 0.0 and abs(delta_latitude_SV) <= 0.1 and TV_position_vector[i,1] < longitude_SV):                                           #7
            dist = ((TV_position_vector[i,1] - longitude_SV)**2 + (TV_position_vector[i,0] - latitude_SV)**2)**0.5
            if debug:
                print "7"
        elif (delta_longitude_SV < 0.0 and delta_latitude_SV < 0.0 and TV_position_vector[i,1] < longitude_SV and TV_position_vector[i,0] < latitude_SV):   #8
            dist = ((TV_position_vector[i,1] - longitude_SV)**2 + (TV_position_vector[i,0] - latitude_SV)**2)**0.5
            if debug:
                print "8"
        else:
            print "SEM DIRECAO DEFINIDA"

        if debug:
            print "Dist: ", dist
            
        #if dist > GPS_RANGE and dist_old > dist:
        if dist_old > dist and dist > 0.0:
            pos = i
            dist_old = dist             #else, update dist_old value
            #break
        #else:
    ...
    #print "TV: ", TV_position_vector[pos-1,0], TV_position_vector[pos-1,1] 
