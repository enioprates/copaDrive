...
def trata_sonar():
	...
	if (range_LF < max_range_LF and range_RF < max_range_RF):
		obstacle = FRENTE
		if range_LF < max_range_LF/2 or range_RF < max_range_RF/2:
			desvio = DESVIO * 2
		else:
			desvio = DESVIO
	elif range_LF < max_range_LF:
		obstacle = ESQUERDA
		if range_LF < max_range_LF/2:
			desvio = - DESVIO * 2
		else: 
			desvio = - DESVIO 
	elif  range_RF < max_range_RF:
		obstacle = DIREITA
		if range_RF < max_range_RF/2:
			desvio = DESVIO * 2
		else:
			desvio = DESVIO 
	elif range_LM < max_range_LM:
		obstacle = ESQUERDA
		if range_LM < max_range_LM/2:
			desvio = - DESVIO * 2
		else:
			desvio = - DESVIO 
	elif range_RM < max_range_RM:
		obstacle = DIREITA
		if range_RM < max_range_RM/2:
			desvio = DESVIO *2
		else:
			desvio = DESVIO 
...
def listener():
	...
	rospy.Subscriber('/car1/side_sonar_left_front_range',Sonar,sonar_side_R)#subscreve ao topicoi e chama a funcao sonar_RF
	rospy.Subscriber('/car1/side_sonar_right_front_range',Sonar,sonar_side_L)#subscreve ao topicoi e chama a funcao sonar_RF
