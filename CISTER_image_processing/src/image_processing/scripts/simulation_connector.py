...
#PID das rodas
kp_steer_l= 18.0						#Kp 40 km - 3.2			kp 50 km 5.5  5				18		18
ki_steer_l= 0.0						#Ki 40 km - 0.01		Ki 50 km 0.01   0			0			3
kd_steer_l= 7.0						#Kd 40 km - 0.2			Kd 50 km 0.4  0.45			7			1

kp_steer_c= 18.0 						#Kp 40 km - 4.0			Kp 50 km 5.5			18			18
ki_steer_c= 0.0					#Ki 40 km - 0.005		Ki 50 km 0.075				0			3
kd_steer_c= 7.0 						#Kd 40 km - 0.25		Kd 50 km 0.25			7			1
...
INITIAL_SPEED = ON			#define initail speed
TARGET_SPEED = 13.00 #target speed (approx 50KM/h)
STOP_VEHICLES = ON			#variable to stop the vehicles
STOP_LATITUDE = 290.0		#OVAL: 290.0	#CURVE: 300.0
STOP_LONGITUDE = 3.75		#OVAL: 3.75		#CURVE: 45.85
STOP_LAP = 2			#time to stop the vehicles
time_base = 0										#start time of the system

LAP_LATITUDE = 160.0		#OVAL:	160.0	#CURVE: 160.0
LAP_LONGITUDE = 3.75		#OVAL:	3.75 	#CURVE:	45.85
...
#-----------------------------------------------------------------------
#FUNCAO: calc_throttle_brake(s_sp, s_real) -> s_sp esta em KM/h e s_real em m/s
...
def calc_throttle_brake(s_sp, s_real):
	...
	standard_speed = 0			#variavel que se torna 1 qdo atinge a velocidade desejada
	
	current_time_TV = tp.time() - time_base  #define time for stop the system
	#current_time_TV = 0.0
	...
	if(abs(accel_control) < EDGE_LOW_SPEED and s_sp == MIN_VEL):		#necessario para zerar a velocidade
		...
		if s_real >= TARGET_SPEED:
			standard_speed = 1
		elif standard_speed == 0 and INITIAL_SPEED:
			aceleracao = 1.0
		#print ("standard speed: ", standard_speed)
		#print ("s_real: ", s_real)
	if STOP_VEHICLES and lap_TV >= STOP_LAP:		#function to stop the vehicles to test the brakes
		if (abs(longitude_real-STOP_LONGITUDE) < 2 and abs(latitude_real-STOP_LATITUDE) < 2) or stop_condition:  	
			aceleracao = 0.0
			freio = 1.0
			stop_condition = ON
...
def car_info(data):
	...
	if (abs(longitude_real-LAP_LONGITUDE) < 1 and abs(latitude_real-LAP_LATITUDE) < 2):  	# conta o numero de voltas
...
def vehicle_control(data):
	...
	#print 'Control_error_steer: ',control_error_steer
