...
def direction_control (lat_TV, long_TV, h_TV, h_SV):
    ...
    if (dt >= MINIMUM_DT):                                  #act only after new controller variables
        ...
        if (abs(theta_error) >= EDGE_ANGLE):                            #If theta_error is significative
            ...
            #---------------------------------------------------------------------------
            #   The maximum steerimg of the car is 20 degrees (0.34 radians)
            #---------------------------------------------------------------------------
...
def vehicle_general_control():
    ...
    global latitude_TV
    global heading_TV
sksk	                #chamada para a variavel global
sksk                   #steering (angulacao das rodas do veiculo em relacao ao veiculo)
sksk                   #percentual do acelerador (0 - 1)
sksk                   #percentual do freio (0 - 1)
sksk
    #SV data
    global longitude_SV
    global latitude_SV
